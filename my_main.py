...
# Contenitori aventi i parametri per effettuare i gradient descent
...
# Valuto la funzione nei punti campionati
...
# Ottengo i miei GP con differenti hyperparametri
...
# Assegno il mio minimo (assoluto) in quello più vicino nel mio dominio
...
for s in range(n_iterations):
    ...
    for i, cpp_gaussian_process in enumerate(gp_loglikelihood.models):  # What are these models?
        ...
        # Valuto la posterior nei punti
        # Prima creo un oggetto Posterior Mean del mio Gaussian process
        ps_evaluator = knowledge_gradient.PosteriorMean(cpp_gaussian_process)

        # Per ogni punto già di cui ho un sample + i 100 nuovi uniformi nel dominio
        for i, pt in enumerate(eval_pts):
            ps_evaluator.set_current_point(pt.reshape((1, gp_loglikelihood.dim)))
            test[i] = -ps_evaluator.compute_objective_function()  # == ComputePosteriorMean()

        # Scelgo il miglior punto (batch) che minimizza la posterior al tempo n
        initial_point = eval_pts[np.argmin(test)]
        
        # Creo un container per effettuare il gradieent descent
